...
void printUsage(const char *progName) {
	std::cout << "\n\nUsage: " << progName << " [options] <scene.pcd>\n\n"
	          ...
	          << "-s <float>   support size for the interest points (diameter "
	             "of the used sphere - "
	             "default "
	          << support_size << ")\n"
...
int main(int argc, char **argv) {
	...
	if(pcl::console::find_argument(argc, argv, "-m") >= 0) {
		...
		std::cout << "Setting unseen values in range image to maximum range "
		             "readings.\n";
	}
	...
	if(pcl::console::parse(argc, argv, "-c", tmp_coordinate_frame) >= 0) {
		coordinate_frame =
		    pcl::RangeImage::CoordinateFrame(tmp_coordinate_frame);
	...
	viewer.addPointCloud(
	    range_image_ptr, range_image_color_handler, "range image");
