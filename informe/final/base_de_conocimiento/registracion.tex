...
En cambio, si el valor de $r$ es demasiado grande,
se corre el riesgo de considerar dentro de la vecindad
una cantidad excesiva de puntos que distorsionen el valor del descriptor o suavicen detalles de la región.
...
\begin{itemize}
    \item $\text{PFH}(p)$ interconecta todos los pares de puntos $\{q_j, q_k\}$ dentro de la vecindad de $p$.
        En cambio, $\text{FPFH}(p)$ interconecta solamente $p$ con sus vecinos.
    \item $\text{FPFH}(p)$ puede incluir puntos fuera del radio $r$ de vecindad, pero a no más de $2r$.
\end{itemize}
