\section{Módulo de registración}
	...
	A continuación se describirán dos métodos para calcular la alineación inicial entre un par de capturas. 
	...
	\subsection{Alineación mediante \emph{sample consensus}}
		\subsubsection{Selección de keypoints}
			...
			Se procedió entonces a cambiar el método de selección de keypoints,
			eligiendo un análisis de persistencia multiescala\cite{Rusu:2009:FPF:1703435.1703733}:
			\begin{enumerate}
	...
	\subsection{Alineación mediante búsqueda de clúster}
		En este método alternativo de alineación no se se seleccionan keypoints,
		simplemente se realiza un submuestreo uniforme de los puntos de la nube
		para reducir el costo computacional.
		Además del descriptor, en cada punto se estableció un marco de referencia
		que nos permite estimar una transformación de alineación
		...
		\begin{figure}
			...
			\caption{\label{fig:marco_referencia_iss}Marcos de referencia. Se observa una ambigüedad equivalente a un giro de $180^{\circ}$ sobre el eje $e^{3}$ \RefImagen{ISS}.}
		\end{figure}
		...
		el eje $y$ y una translación en el plano $xz$.  Se observa entonces
		una agrupación de los parámetros de estas transformaciones y, mediante el
		algoritmo de k-means, se busca el centroide del clúster más grande (figura~\ref{fig:cluster}).
		\begin{figure}
			\resizebox{.9\linewidth}{!}{\input{diagram/cluster_tikz}}
			\caption{\label{fig:cluster}Translaciones estimadas entre cada par
			de puntos de las correspondencias.\TODO{unir con histograma de ángulos (cubeta de $2^{\circ}$}}
		\end{figure}
